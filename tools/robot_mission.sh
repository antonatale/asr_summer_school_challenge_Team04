...
case "${1:-help}" in
  launch)
    exec timeout --signal=INT --kill-after=10s 420s ros2 launch asr_summer_school autonomy.launch.py sensors:=true "duration:=${2:-60.0}" "max_radius:=${3:-1.0}"
    ;;
  ...
  stop)
    ...
    ;;
  *) echo 'Usage: robot_mission.sh launch [seconds=60] [radius_m=1] | status | snapshot | arm | start | stop';;
esac
